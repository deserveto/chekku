...
- Starting and observing a run are distinct operations. ChatStudio (`client/src/lib/agent-runs.ts`) discovers a thread's active run on mount and subscribes with offset-based replay (exactly-once across reconnects); it must never restart the prompt when reconnecting.
...
- the agent run lifecycle: registry concurrency (409 on duplicate non-terminal runs), cancel idempotency, event replay from offset, buffer eviction and terminal TTL, ownership collapse to 404, execution driver terminal states (completed/failed/cancelled), server-side first-turn thread creation and titling at run start (`ensureFirstTurnThread`), run summaries carrying the prompt, the `/api/runs` seam (resourceId injection, thread-ownership rejection), SSE parsing and offset reconnect in `client/src/lib/agent-runs.ts`, and ChatStudio reconnection behavior (active-run discovery on mount with prompt synthesis, no duplicate start on 409, navigation unblocked while running, run-scoped stop, thread-list refresh on start, sidebar running indicator);
