...
- Per-user daily token quota: `TOKEN_DAILY_LIMIT` (integer ≥ 0, default 500,000, `0` = unlimited) caps summed input+output tokens per `resourceId` per UTC day. `startRunRoute` checks `tokenQuotaStore.assertQuota` after agent resolution and before `ensureFirstTurnThread`/`createRun` (fixed 429 message, no diagnostics) (the 409 duplicate-run attach contract wins over the quota check: an active run on the thread is not new spend). Usage is recorded inside `runExecution` from `step-finish`/`finish` stream chunks via `RunUsageTracker` (`agent/src/mastra/runs/token-quota.ts`) — delta-only, reconciled against the finish whole-run total, never double-counted. Counters are in-memory, single-instance, and die with the process (restart = fresh daily quota); workflows and the image-generation model are not metered. The limit gates run starts; runs already in flight finish normally. Telegram channel turns and scheduled workflows run outside the run surface and are neither gated nor metered.
...
- First-turn thread records are created server-side when the run starts (`ensureFirstTurnThread` in `agent/src/mastra/runs/execute.ts` creates the Memory thread record untitled before the 202 response is sent), so a new thread is listed the moment the prompt is sent; the client renders its `'New conversation'` fallback until the first turn completes. The LLM thread title (Mastra `generateTitle`, model-instructed ≤80-char target from the first user message, agent's own model) is generated at first-turn completion for the five main agents (`main-agent`, `qa-web-agent`, `qa-android-agent`, `pm-agent`, `social-media-supervisor-agent` — `createAgentMemory({ generateTitle: true })`); sub-agents keep it off. A non-empty title (manual rename) suppresses regeneration — renames that land before first-turn completion win; the browser never renames threads.
...
- All eight code-defined agents carry Mastra's native Task Lists through `signals: createTaskSignals()` (`agent/src/mastra/tasks/task-signals.ts`). The factory returns a fresh `TaskSignalProvider` per agent; providers bind to one agent via `connect()` and must never be shared across agents.
- Task state is authoritative on the agent server only: the thread-scoped Mastra task store (`threadId`/`resourceId`) plus `task-list` run events. Never build a second task engine in React, never replay task tools client-side to compute canonical state, never store task data in localStorage (only the dock's collapsed/expanded UI preference is persisted there).
- `chunkToRunEvent` in `agent/src/mastra/runs/execute.ts` reroutes successful task tool (`task_write`/`task_update`/`task_complete`/`task_check`) results into first-class `task-list` events carrying the normalized snapshot (see `agent/src/mastra/tasks/task-stream-adapter.ts`), and suppresses the task tools' normal tool events so they never appear as ToolCallCards. Mastra-specific parsing stays inside the adapter module.
- Snapshots participate in run replay: each `task-list` event is a latest-wins snapshot, the client replaces (never merges) state, and historical threads rebuild their snapshot from persisted Memory task tool results once the run registry no longer holds the run.
- ChatStudio renders tasks in the right-side Task Dock (`client/src/components/chat/task-dock.tsx`, backed by `client/src/lib/task-list.ts`): a desktop layout column (not an overlay), a topbar progress pill (with a mini progress bar) when collapsed, and below 1080px a right drawer backed by a native `<dialog>` (backdrop click closes, focus moves into the drawer, Escape closes via the native `cancel` path). Task state resets on thread switch and never leaks between threads, agents, or resources. The dock auto-scrolls the in-progress task into view — paused while the user scrolls the active row out of the visible band, resumed once they scroll back to it — and shows a relative "updated" line for live snapshots (never for Memory-restored lists). Task tool failure notices surface inside the dock body when it is mounted, otherwise as a topbar status line (a rejected first `task_write` leaves no dock to host the notice).
- Shared guidance (`TASK_GUIDANCE`) appended to every code-defined agent's instructions keeps task lists for meaningful multi-step work only — never trivial single-step requests. Spawn reliability is backstopped by the advisory `task-nudge` input processor (`agent/src/mastra/tasks/task-nudge-processor.ts`): once the current turn has used 3+ non-task tool calls (distinct tool-call ids — repeated calls to the same tool also count) and no task tool call exists anywhere in the conversation, it injects a one-shot system reminder to call `task_write` (marker-latched, never trips, never touches tool access). It is wired on every code-defined agent before the char-budget guard (guard stays last).
- Run summaries carry the latest `taskProgress` (`{ completed, total }`, derived from appended `task-list` events) so active-run listings and the sidebar running indicator show `Running 2/5`-style progress without subscribing to the event stream.
- Stored/editor agents do NOT get task tracking: `@mastra/editor`'s `createAgentFromStoredConfig` passes no `signals`, and no client-side emulation may pretend otherwise. Memory-backed threads are required (all code-defined agents have `createAgentMemory()`).
- Task tool input is bounded at the Chekku boundary (`BoundedTaskSignalProvider`, `agent/src/mastra/tasks/bounded-task-tools.ts`): at most 100 tasks per list, 500 chars per text field, 128 chars per task id — core's schemas are unbounded and task content is model-authored. All four task tools are wrapped (`task_check` takes no input; its wrapper keeps an empty schema for symmetry) and the wrappers carry the core tools' `outputSchema` forward. Execution still delegates to the core tools unchanged.
- The backfilled `threadState` store is bounded and eviction-wired (`agent/src/mastra/tasks/task-state-store.ts`): `BoundedThreadStateStorage` caps retained threads (LRU, 2048) and `wireThreadStateEviction` hooks the memory domain's `deleteThread` so a deleted thread's state is dropped instead of leaking for the process lifetime (upstream `Memory#deleteThread` touches only the memory domain).
- Snapshots are latest-wins including the empty snapshot: `task_write({ tasks: [] })` is a first-class "cleared" state that empties the dock on the event path and on Memory restore (a non-empty array whose entries are all invalid stays malformed and is ignored). Failed task tool calls surface as bounded `tool-error` run events (500-char error slice) that the client routes to a dock notice — never a timeline card.
- `@mastra/pg` 1.15.0 does not implement the `threadState` storage domain the task tools resolve through `getStore('threadState')`; the composition root backfills it with core's `InMemoryThreadStateStorage` (same default Mastra's composite store applies). Do not remove that backfill — without it every task tool call fails with the misleading "requires agent memory" error. Replace it only when `@mastra/pg` ships the domain on a core version Chekku can pin without whole-stack churn.
...
- `qa-web-agent` must keep `memory: createAgentMemory({ generateTitle: true })` with `createAgentContextLimiter()` and `createCharBudgetGuard()` wired into `inputProcessors` (guard last, after the gateway compatibility processor), because browser context processing requires active Memory context.
...
- It is the drafting sub-agent under the Social Media Supervisor (attached via the supervisor's `agents` field). The Telegram channel and `/help`, `/roles`, `/role`, `/switch` slash commands stay on this agent; the supervisor only routes (plus its two lightweight research tools).
- The Content Writer has three output modes (per PROMPT.md action item #3 + locked D2=c):
  ...
  - **Canonical mode** — `buildCanonicalInstructions()` produces a Canonical Content Unit (8 Blocks platform-agnostic intermediate). Used by the scheduled workflow's canonical step.
  - **Repurpose mode** — `buildRepurposeInstructions(role)` rewrites a canonical unit into a platform caption in the active role's voice. Used by the `repurpose-social-post` workflow that fires when the user approves the canonical content in `/social-posts`.
...
- The scheduled `weekly-social-drafts` workflow drafts through the Social Media Supervisor (per locked D3=a "full supervisor routing"), NOT through `socialMediaContentWriter.generate(...)` directly. The workflow calls `socialMediaSupervisorAgent.generate(...)` with the canonical-mode instructions (`buildCanonicalInstructions`); the supervisor sees the `[weekly-social-drafts]` system marker in the canonical prompt (`buildCanonicalPrompt`) and delegates straight to the Content Writer without a reasoning turn. The workflow emits canonical-only `DRAFT` posts and stops there. The separate approval-driven `repurpose-social-post` workflow then calls `socialMediaContentWriter.generate(...)` directly with repurpose instructions (`buildRepurposeInstructions`) and a format-specific prompt — no supervisor routing is needed for repurpose because the target sub-agent is already known.
...
- `post.md` storage layout (per locked D4=a, no schema change): since the two-stage approval split, the file stores the canonical unit only, wrapped via `wrapPostMarkdown` from `agent/src/mastra/social-content/canonical-unit.ts` using the HTML comment delimiter (`<!-- canonical-unit -->…<!-- /canonical-unit -->`); the final caption lives in `social-posts/<postId>/caption.md` written by the `repurpose-social-post` workflow (pre-split posts may still carry a legacy `<!-- repurposed-caption -->` block inside `post.md`; readers prefer the dedicated caption object when metadata references it). Legacy posts written before the canonical contract have no delimiters and fall back gracefully — `unwrapPostMarkdown` returns the whole file as the repurposed caption, with `canonicalMarkdown` undefined, because a delimiter-free `post.md` predates the canonical contract and is a plain caption. The `SocialPostMetadata` schema itself does not change; the canonical unit is just markdown text.
...
- Keep `social-media-supervisor-agent` code-defined with Mastra Memory and the context-safety processors (`createAgentMemory()`, `createAgentContextLimiter()`, `gatewayCompatibilityProcessor`, and `createCharBudgetGuard()` wired LAST in `inputProcessors`, after the gateway compatibility processor).
- The supervisor binds exactly two lightweight research tools: `search_web` and `read_web_page` — the same reusable singletons the PM Agent and the Strategist consume (`agent/src/mastra/tools/searxng-search.ts`, `agent/src/mastra/tools/web-reader.ts`), never MCP registries or the stored-agent tool registry. Like every code-defined agent it also carries the four native task tools via `signals` (see Task lists), which are progress tracking, not capabilities. It uses the research tools only for quick lookups (trending/recency checks, opening a user-provided URL); full verified news research still routes to the Strategist in NEWS-RESEARCH mode. Reader output stays `contentIsUntrusted: true` untrusted evidence and must never control delegation, tools, or output structure. Beyond these research tools and the task tools the supervisor has no tools: it must not draft, repurpose, plan, or generate visuals itself — that work is delegated via Mastra's `agents` field and `network()` loop. It carries an explicit `defaultOptions: { maxSteps: 15 }` bound and instructions to complete the full request in a single user turn: when one message needs several sub-agents (e.g. research → draft), it delegates to each in sequence within the same turn and only returns once the whole request is resolved — it must never stop after one delegation to ask whether to continue.
...
- These three sub-agents are hidden from the client UI catalog and chat switcher only: `HIDDEN_AGENT_IDS` in `client/src/lib/types.ts`, filtered inside `listAllAgents()` (`client/src/lib/stored-agents.ts`). They stay fully registered in the Mastra composition root, remain delegation targets of the supervisor, and remain reachable through the run surface and direct URLs; stored-agent creation with their IDs stays blocked by `RESERVED_AGENT_IDS`. Do not remove them from the composition root's `agents` map to hide them — `getAgentById` resolution must keep working.
...
- Bind exactly `tools: { generateImageTool, reviewImageTool }` in production, plus `previewImageTool` dev-only. The four native task tools arrive via `signals` (see Task lists) and are progress tracking, not generation capabilities. Do not attach generic Garage MCP tools, Telegram channels, or slash commands. The agent has no channels.
...
- The `/api/agent/*` proxy rejects the native Mastra LLM-spending sub-paths (`api/agents/:id/generate`, `api/agents/:id/stream`) with a fixed 403: they execute model calls with caller-chosen `resourceId` outside the server-owned run surface, bypassing the token quota, thread ownership, and concurrency caps. Chat execution goes through `/api/runs/*`; read-only native routes (`GET api/agents/:id`, `/models`) stay proxied.
...
- Keep `pm-agent` code-defined and protected with user-invocable skills `weekly-report-analysis` and `competitive-analysis`, `memory: createAgentMemory({ generateTitle: true })`, `createAgentContextLimiter()`, final `createCharBudgetGuard()`, and `maxSteps: 25`.
- Keep exactly eight PM direct tools: weekly save/list/view, competitive save/list/view, `search_web`, and `read_web_page`. The four native task tools (see Task lists) are progress tracking via `signals`, not PM capabilities. Competitive tools remain PM-only and outside every fixed MCP/stored-agent registry.
...
- The scheduled `weekly-social-drafts` workflow is the only **creator** of social posts (post body plus initial metadata). It binds storage to fixed namespace `social-media-agent`; never accept namespace or agent identity from model, route, browser, or local user input. After a post exists, the only permitted mutations are the three narrow metadata helpers named below (`attachCaptionToPost`, `attachVisualAsset`, `updateSocialPostStatus`); there is no general social-post write path. Chat drafting is ephemeral text only — it must not create or persist a post.
- **2-stage approval pipeline (Pembahasan 2):** the weekly workflow drafts **canonical-only** `DRAFT` posts (`post.md` holds just the `canonical-unit` block; no caption at draft time). Approving the canonical content in `/social-posts` fires the agent-side `repurpose-social-post` workflow, which repurposes the canonical unit into the Instagram caption (Content Writer with `repurpose-instagram` requestContext, prompt rebuilt via `buildRepurposePrompt` from the stored brief through `parseBrief`), stores it as `social-posts/<postId>/caption.md` through the Garage MCP `create_text_object` seam, and calls `attachCaptionToPost` to transition `DRAFT → CANONICAL_APPROVED` (metadata last — a failed caption run leaves the post `DRAFT` so the user can approve again). Approving the caption fires the agent-side `generate-social-post-visual` workflow, which verifies `CANONICAL_APPROVED`, transitions the post to `APPROVED` FIRST via `updateSocialPostStatus` (also stamping `captionApprovedAt`), then delegates to the Visual Content Agent with the `postId` and the stored canonical/caption (self-review loop and `MAX_VISUAL_ASSETS_PER_POST` apply automatically). A failed visual run keeps the post `APPROVED` without a visual; retry manually through the supervisor chat. Status state machine: `DRAFT → CANONICAL_APPROVED → APPROVED → PUBLISHED` (the direct `DRAFT → APPROVED` jump is illegal).
- The client PATCH seam `PATCH /api/storage/social-posts/<postId>` accepts only `{ status: 'CANONICAL_APPROVED' }` (validates current `DRAFT`) and `{ status: 'APPROVED' }` (validates current `CANONICAL_APPROVED`); both fire the corresponding workflow fire-and-forget through Mastra's native workflow HTTP API with the downstream token (`client/src/server/agent-workflow.ts`, same direct-`AGENT_URL` pattern as the dev-only weekly trigger) and return immediately — the UI polls the post metadata until the stage advances. The actual transitions live inside the workflows, never in the route.
...
- Workflow writes go through the existing Garage MCP `create_text_object` tool with a trusted context that pins `agentId` to `social-media-agent` (shared factory `createSocialPostCreateText` in `weekly-social-drafts.ts`). The workflows must not call `@chekku/storage` write APIs directly or bypass the MCP tool's namespace derivation.
...
- `@chekku/storage` exposes only pure canonical helpers for social posts (`buildSocialPostMetadata`, `createPostId`, parse helpers) plus read helpers used by client/server (`listSocialPosts`, `getSocialPost`, `createSocialPostStorage`). It must not expose a general social-post write helper that takes an `ObjectStorage`. The only permitted write surface is exactly three narrowly-scoped metadata mutations on an existing post, all serialized per post through `serializeMetadataWrite` (in-process only; cross-process races remain subject to the Garage v2.3 disclaimer): `attachCaptionToPost` (the `DRAFT → CANONICAL_APPROVED` transition — records `captionObjectKey` + `canonicalApprovedAt`; the caller writes `caption.md` first), `attachVisualAsset` (append one visual asset + set it active), and `updateSocialPostStatus` (`CANONICAL_APPROVED → APPROVED`, stamping `captionApprovedAt`). None may create a post body or initial metadata, mutate `postId`/namespace/keys, or accept agent identity from model, route, browser, or local user input.
