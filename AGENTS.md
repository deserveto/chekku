...
- Development and production are kept apart by Compose profiles, not by separate files. The `agent` and `client` services in `compose.yaml` are gated behind `profiles: [prod]`. `scripts/dev.sh` never activates the `prod` profile and continues to run the agent and client as host processes with only Garage, SearXNG, and Postgres in containers. `scripts/prod.sh` is the only path that activates the profile and brings the whole stack up as containers.
...
- Mastra currently inlines `@chekku/storage` into the agent bundle, but the agent runtime stage keeps `WORKDIR /app` and copies the `storage/` workspace so the runtime layout matches the build layout even if a future Mastra version externalizes storage via an absolute path.
