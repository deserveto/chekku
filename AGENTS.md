...
- ChatStudio renders tasks in the right-side Task Dock (`client/src/components/chat/task-dock.tsx`, backed by `client/src/lib/task-list.ts`): a desktop layout column (not an overlay), a topbar progress pill (with a mini progress bar) when collapsed, and a right drawer below 1080px (scrim tap-to-close, focus moves into the drawer, Escape closes). Task state resets on thread switch and never leaks between threads, agents, or resources. The dock auto-scrolls the in-progress task into view and shows a relative "updated" line for live snapshots (never for Memory-restored lists).
