...
- the agent run lifecycle: registry concurrency (409 on duplicate non-terminal runs), cancel idempotency, event replay from offset, buffer eviction and terminal TTL, ownership collapse to 404, execution driver terminal states (completed/failed/cancelled), server-side first-turn titles, the `/api/runs` seam (resourceId injection, thread-ownership rejection), SSE parsing and offset reconnect in `client/src/lib/agent-runs.ts`, and ChatStudio reconnection behavior (active-run discovery on mount, no duplicate start on 409, navigation unblocked while running, run-scoped stop, sidebar running indicator);
