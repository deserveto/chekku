...
- Run event buffers are bounded (4 MiB / 10 000 events per run, oldest-eviction with an `evicted` flag) and terminal runs are retained 30 minutes for replay. The run-status registry is in-memory and single-instance: an agent server restart kills the HTTP run surface (status, replay, 409 locks) together with the process. The single sanctioned exception to "no durable run storage" is the pm-agent durable pilot below: its execution snapshots persist through the workflow engine into Postgres, but they never feed the HTTP run surface and carry no automatic recovery.
- Durable execution pilot (N8_4): `pm-agent` is wrapped with `createDurableAgent` (`durablePmAgent` in `agent/src/agents/pm-agent.ts`, registered in the composition root under key `pmAgent`; public id stays `pm-agent` so `getAgentById`, thread-id ownership, and the `/runs` surface are unchanged). The same `agent.stream(...)` call now runs the agentic loop inside Mastra's built-in workflow engine using the default in-process PubSub and in-memory event cache — no Redis, no `mastra.pubsub` configuration. Stop is unchanged (the registry's AbortController signal is forwarded to the durable run's internal controller); resume is conversational — prompting again in the same thread continues from persisted Memory context, made possible by the abort persistence bridge in `agent/src/mastra/runs/execute.ts`: when a run reaches its cancelled terminal state, the driver reconstructs the partial turn from registry events (user message from the prompt, tool calls with bounded results — capped at 6,000 chars per tool result and 48,000 chars per turn — synthetic interrupted tool-results for unpaired tool-calls so the next provider request stays valid, and the streamed text with a visible stop marker) and persists it best-effort via `memory.saveMessages` (insert, not update — a cancelled turn has fresh message ids, so an update-only call silently drops it). Two restore-shape invariants: synthetic interrupted tool-results carry `interrupted: true` on the part while the `error-text` output stays for provider validity, so the client restore path (`restoreAssistantParts` / `collectFlatToolResults`) renders the stopped card as `interrupted`, never as an error; and the user row is timestamped one millisecond before the assistant row, because the Postgres store tie-breaks equal `createdAt` by message id — `${runId}-assistant` sorts above `${runId}-user`, which swapped the restored bubble order (assistant above the prompt) after a page refresh. Completed runs still persist through Mastra alone, and failed runs persist nothing (follow-up). The execution driver calls the stream result's `cleanup()` once the run is terminal so PubSub subscriptions and engine registry entries never leak. Known engine limitation (pinned `@mastra/core` 1.50.1): the durable tool-execution step does not forward the abort signal into `tool.execute`, so an in-flight tool call (e.g. `read_web_page`, bounded to 30 seconds) runs to completion and the abort lands at the next LLM step; the client compensates optimistically by flipping running tool cards to `interrupted` the moment the user requests a stop. Crash recovery stays disabled (`recoverActiveRuns` / `recovery.durableAgents` do not exist in the pinned `@mastra/core` 1.50.1, and re-driving abandoned runs would re-issue real LLM calls). Every other agent still registers its plain instance; rolling more agents onto durable execution is a separately reviewed change.
...
- Native alternatives were evaluated and rejected for the run surface: `observe()` replay and the `sendMessage`/`subscribeToThread`/signals surface are not a fit — the original rejection predates `@mastra/core` 1.50.1, whose durable agents ship default in-process PubSub plus an in-memory event cache (sufficient for single-instance; the pm-agent pilot above uses exactly those defaults). The custom run manager is retained regardless because it — not `observe()` — owns thread-ownership checks, 409/429 caps, the watchdog, and terminal replay. It uses only stable primitives (`agent.stream` with caller-supplied `runId` and `abortSignal`, `mastra.getAgentById`). `getAgentById` resolves by the agent's public id first and falls back to a registry-key lookup, so stored agents registered at runtime resolve through the run surface too; prefer it over `getAgent` because the public id is what clients send and composition keys (`mainAgent`, ...) are internal.
...
- Bind exactly `tools: { generateImageTool, reviewImageTool, previewImageTool }` in every environment (production included). Do not attach generic Garage MCP tools, Telegram channels, or slash commands. The agent has no channels.
