...
- Shared guidance (`TASK_GUIDANCE`) appended to every code-defined agent's instructions keeps task lists for meaningful multi-step work only — never trivial single-step requests. Spawn reliability is backstopped by the advisory `task-nudge` input processor (`agent/src/mastra/tasks/task-nudge-processor.ts`): once the current turn has used 3+ distinct non-task tools and no task tool call exists anywhere in the conversation, it injects a one-shot system reminder to call `task_write` (marker-latched, never trips, never touches tool access). It is wired on every code-defined agent before the char-budget guard (guard stays last).
