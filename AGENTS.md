...
- **Brand asset (`agent/src/assets/image.png`):** the SINGLE canonical Rafiqspace logo. The compositor loads it via `loadBrandLogoBytes()` and stamps it onto every generated visual as-is — never regenerated, never redrawn, never substituted with wordmark text. Resolution order: `agent/src/assets/image.png` (repo root CWD), `src/assets/image.png` (agent workspace CWD), then `process.cwd()/src/assets/image.png`. Tests inject a fixture logo via the `logoPath` option on `createPreviewImageTool` / `createGenerateImageTool`.
