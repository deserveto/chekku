...
| `npm run eval:qa-web` | Run the on-demand Mastra eval for the QA Web Agent's read-only browser smoke-test path: one local fixture case, browser-trajectory and report-structure gates, and an LLM-judge score. Requires the model gateway plus Chromium (`npx playwright install chromium`); no Docker service or remote website. Not part of `npm run check` or CI. |
...
The agent probe suite (`e2e/agents/agent-probe.spec.ts`) is a live integration check of the studio's built-in agents. It signs in one fresh test user, reads the agent list off `/agents` itself — so a newly added built-in is picked up without editing the spec — then, for each built-in agent, clicks **Open chat**, sends the single probe prompt `Apa yang kamu bisa lakukan untuk saya?`, and asserts the run finishes with a non-empty, error-free reply. Stored (user-created) agents are listed in the console output but not probed: opening one can migrate its model gateway in place and run its own tools, so an automated sweep must not mutate someone's custom agent or fail the suite over one that is half-configured. Each built-in agent is a reported step, and a failing agent does not stop the ones after it: the test ends by listing every agent that failed. It performs real LLM calls through the agent server, so the agent server must be running with a configured model gateway (`npm run dev:agent`, `npm run dev`, or an already-running stack) before invoking it; the per-agent budget is derived from the probe's phase caps plus headroom, so a full sweep can take several minutes.
...
On-demand quality evals live under `agent/src/evals/` and run through a dedicated Vitest config (`vitest.eval.config.ts`), so `.eval.ts` files never execute inside `npm test`, `npm run check`, or CI. Each command targets one agent eval and makes live LLM calls through the production model gateway. The social eval uses two self-contained brand-strategy cases and compares the Content Strategy Brief against committed golden references through a deterministic structure gate plus an LLM judge. The QA Web eval uses one local read-only fixture case: the trajectory gate requires navigation, snapshot, and click actions; the report gate requires the structured QA report; and the LLM judge scores task coverage, evidence accuracy, scope safety, and report clarity against its committed golden reference. Both commands use in-memory Mastra storage and fresh evaluation threads; no Postgres, Garage, SearXNG, Reader, Docker service, or remote website is required. Configure `LLM_BASE_URL`, `LLM_API_KEY`, and `LLM_DEFAULT_MODEL` in `agent/.env`; install Chromium once with `npx playwright install chromium` for the QA Web eval.

Security note for `npm run eval:qa-web`: the local fixture removes the remote-site dependency, but the eval still launches a real headless Chromium with unrestricted outbound network egress. Prompt restrictions and post-run trajectory checks are not a network sandbox. Run it only in a trusted environment and do not use a browser profile containing sensitive credentials.
