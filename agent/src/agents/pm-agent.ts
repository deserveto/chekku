...
import { listPmReportsTool, savePmReportTool, viewPmReportTool } from '../mastra/tools/pm-report-tools.js';
...
Call the list_pm_reports tool. Present a concise list with reportId, createdAt, risk rating, and status. If there are no reports, say no saved reports were found.

## When the user asks to view, read, open, or show a saved report

If the user provided a report id, call the view_pm_report tool with that reportId. Return the saved analysisMarkdown first, then a short metadata block with reportId, createdAt, rating, and status. If no report id was provided, ask for the reportId or offer to list saved reports.
...
- After writing the Markdown analysis, call save_pm_report with original report as reportMarkdown and your analysis as analysisMarkdown. In final response, include the Markdown analysis and "Saved reportId: <reportId>".
- If save_pm_report fails, still return the Markdown analysis and add one short line explaining local report save failed.
...
const pmAgentConfig: AgentConfig<string, ToolsInput, undefined, ProviderContext> = {
  id: 'pm-agent',
  name: 'PM Agent',
  description: 'Analyzes engineering weekly reports, rates project risk, and saves report analyses locally.',
  model: () => getServerModel(),
  requestContextSchema: providerContextSchema,
  tools: {
    save_pm_report: savePmReportTool,
    list_pm_reports: listPmReportsTool,
    view_pm_report: viewPmReportTool,
  },
  memory: new Memory(),
