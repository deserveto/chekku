...
- **Durable execution (pm-agent pilot + Task D rollout).** `main-agent`, `pm-agent`, and `qa-web-agent` are registered as `createDurableAgent({ agent })` wrappers (public ids and composition keys unchanged), so the longest jobs — competitive research (up to 25 steps) and browser QA sessions (up to 80 steps) — run inside Mastra's built-in workflow engine with the default in-process PubSub and an in-memory event cache — no Redis. The run surface is unchanged: stop still flows through the run signal (forwarded to the durable run's internal controller), replay still comes from the run registry, and the execution driver calls the durable stream's `cleanup()` at terminal state so PubSub subscriptions and engine registry entries never leak. Workflow snapshots persist to Postgres through the shared `PostgresStore`, but crash recovery is not enabled (unavailable in the pinned `@mastra/core` 1.50.1, and re-driving abandoned runs would re-issue real LLM calls): a restart still abandons in-flight runs, and resume is conversational — prompting again in the same thread continues from the last persisted Memory context. `social-media-content-writer` (Telegram channels are not carried by the wrapper), `qa-android-agent`, the social cluster, and stored agents stay on plain instances.
