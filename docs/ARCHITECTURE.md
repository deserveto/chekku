...
**Retrieval** (`agent/src/mastra/tools/knowledge-search.ts`, `search_knowledge_base`, bound to `main-agent` only): the tool takes `{ query, limit? }` — no tenant field exists in the input schema. The trusted `resourceId` comes from `context.agent.resourceId` (populated by Mastra from the run's memory options, the same trusted seam the Garage MCP uses for `agentId`), and every search carries a mandatory `resourceId` equality filter. Results are bounded (default 5, max 8, ≤1,024-byte queries) and carry document/chunk metadata. Retrieved chunks are reference material, not instructions; the Main Agent instructions say so explicitly.
