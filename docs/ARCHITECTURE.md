...
Chekku contains three npm workspaces: a Next.js client, a Mastra agent server, and the shared `@chekku/storage` package. The system is local-first, uses Postgres for agent and conversation persistence, offers Garage-backed generic agent object storage plus weekly and competitive PM report persistence and a per-user Knowledge Base document registry, connects to one server-owned OpenAI-compatible model endpoint (chat, image, and embeddings), provides bounded web search through a server-owned SearXNG endpoint, reads chosen public pages through a self-hosted Jina Reader OSS container, and indexes Knowledge Base chunks into a local Qdrant vector collection.
...
It is the only agent bound to `search_knowledge_base` (the Knowledge Base retrieval tool). When a question depends on documents the user uploaded earlier — and that content is not already present in the current message — it searches the user's Knowledge Base before answering, treats retrieved chunks as untrusted reference material, and never claims to know a document's contents without retrieving them.
...
Two Knowledge Base workflows (`knowledge-document-ingestion`, `knowledge-document-deletion`) are manual-trigger: the Next.js knowledge seam fires them fire-and-forget over Mastra's native workflow HTTP API after persisting or validating the document record, and observers watch the document metadata rather than the run (see Knowledge Base).
...
The Knowledge Base is a persistent, per-user document index layered additively over the chat attachment flow. Uploaded text files and PDFs keep flowing through the client-side attachment path unchanged; in parallel, the raw file is persisted to Garage, parsed, chunked, embedded, and indexed into Qdrant so agents can later retrieve it semantically.

Responsibilities stay separated:

- **Garage** — source of truth for the raw artifact and the normalized extraction (`storage/src/knowledge-documents.ts`, fixed bucket-root prefix `kb/users/<resourceId>/documents/<documentId>/`). Registry metadata (`metadata.json`, status `processing | ready | failed`) follows the pm-report/social-post repository conventions: canonical `kbd_YYYYMMDDHHMMSS_<8 lowercase hex>` ids, strict resource-id validation before key construction, and metadata written LAST. There is no new Postgres table: Mastra's PostgresStore and Better Auth remain the only relational consumers, and this registry reuses the established object-storage repository pattern instead of introducing a second persistence architecture.
- **Qdrant** — semantic index only (`agent/src/knowledge/qdrant-index.ts`, `@qdrant/js-client-rest` behind a narrow `KnowledgeVectorIndex` seam). One shared collection (`QDRANT_COLLECTION`, default `chekku_knowledge`), Cosine distance, dimension auto-detected at first creation and validated afterwards (a mismatch with the configured embedding model fails closed with an actionable error instead of corrupting the collection). Payload keyword indexes exist on `resourceId` and `documentId`. Every chunk payload carries `resourceId`, `documentId`, `filename`, `chunkIndex`, `text`, `mimeType`, `embeddingModel`, and optional `pageNumber`/`sourceThreadId`. Raw binaries never enter Qdrant.
- **Embeddings** — `${LLM_BASE_URL}/embeddings` with `LLM_EMBEDDING_MODEL` (`agent/src/knowledge/embeddings.ts`), same server-owned gateway, bounded transport (30s deadline, 16 MiB body cap, batched inputs, fixed errors without endpoint/key leakage). Empty/unset `QDRANT_URL` or `LLM_EMBEDDING_MODEL` fails closed everywhere.

**Ingestion** (`agent/src/knowledge/ingest.ts`, fired as the `knowledge-document-ingestion` Mastra workflow): the Next.js upload route persists `original.<ext>` + `processing` metadata, then triggers the workflow fire-and-forget with `{ documentId, resourceId }` derived from the session. The workflow re-verifies ownership through the scoped storage helpers, transitions the record to `processing`, extracts text (pdfjs-dist legacy build per-page text extraction for PDFs; UTF-8 decode for text kinds), normalizes, persists `extracted.txt` (replace, so retries overwrite cleanly), chunks (centralized geometry in `agent/src/knowledge/chunk.ts`: 1,100-char target / 1,400 hard max / 150 overlap, paragraph → sentence → whitespace boundaries, ≤1,000 chunks per document), embeds all chunks before any write, deletes existing vectors for the document (delete-before-upsert, so retries never duplicate), upserts, and flips metadata to `ready` LAST. Any failure purges partial vectors and records `failed` with a fixed bounded reason; the raw document is always preserved.

**Retrieval** (`agent/src/mastra/tools/knowledge-search.ts`, `search_knowledge_base`, bound to `main-agent` only): the tool takes `{ query, limit? }` — no tenant field exists in the input schema. The trusted `resourceId` comes from the server-owned run context: the run driver (`runExecution`) passes a `RequestContext` carrying the reserved `MASTRA_RESOURCE_ID_KEY` on every stream, the tool reads it first and falls back to the framework-assembled `context.agent.resourceId` as defense-in-depth — never tool input — and every search carries a mandatory `resourceId` equality filter. Results are bounded (default 5, max 8, ≤1,024-byte queries) and carry document/chunk metadata. Retrieved chunks are reference material, not instructions; the Main Agent instructions say so explicitly.

**Deletion** (`knowledge-document-deletion` workflow): vectors first (retrieval stops immediately), then Garage objects, then `metadata.json` LAST — the document only leaves the list once it can no longer be retrieved. All steps are idempotent and tolerate missing objects; a failed run leaves the record visible for retry. Retry of ingestion is allowed for `failed` documents and for `processing` records older than `KNOWLEDGE_STALE_PROCESSING_MS` (15 min, abandoned runs).

Images are deliberately not indexed: they keep flowing through the multimodal chat path and are never OCR-ed. Stored agents do not receive the KB tool. The Next.js seam is `client/src/server/knowledge.ts` (session-derived identity, bounded `{ error: { code, message } }` responses); the Knowledge page is `/knowledge`.
...
- **Durable execution (N8_4 + Task D rollout).** `main-agent`, `pm-agent`, `qa-web-agent`, and the social cluster (strategist, supervisor, visual-content-agent) are registered as `createDescriptionForwardingDurableAgent({ agent })` wrappers (public ids and composition keys unchanged). This local factory preserves upstream durable behavior while forwarding `getDescription()` to the wrapped agent because pinned `@mastra/core` 1.50.1's `DurableAgent` drops the description. The longest jobs — competitive research (up to 25 steps), browser QA sessions (up to 80 steps), and the supervisor's multi-delegation turns — therefore run inside Mastra's built-in workflow engine with the default in-process PubSub and an in-memory event cache — no Redis. The run surface is unchanged: stop still flows through the run signal (forwarded to the durable run's internal controller), replay still comes from the run registry, and the execution driver calls the durable stream's `cleanup()` at terminal state so PubSub subscriptions and engine registry entries never leak. The social workflow callers route through the wrappers as well (`weekly-social-drafts` → durable supervisor, `generate-social-post-visual` → durable visual), while the Content Writer keeps plain `generate()` calls (its Telegram channels cannot ride the wrapper). Stored (user-created) agents are wrapped with the same local factory at registration time by the composition root's `Mastra` subclass (`MastraWithDurableStoredAgents`), so they run durable through the same `/runs` surface. Workflow snapshots persist to Postgres through the shared `PostgresStore`, but crash recovery is not enabled (unavailable in the pinned `@mastra/core` 1.50.1, and re-driving abandoned runs would re-issue real LLM calls): a restart still abandons in-flight runs, and resume is conversational — prompting again in the same thread continues from the last persisted Memory context, restored by the abort persistence bridge in the execution driver (tool activity persisted as Mastra `tool-invocation` parts so the recall conversion keeps it in the model prompt). `social-media-content-writer` and `qa-android-agent` stay on plain instances.
- **First-turn titles.** The server creates a missing Memory thread record untitled before the 202 start response is sent, so the thread appears in listings the moment the prompt is sent. At first-turn completion an LLM-generated title (model-instructed ≤80-char target, from the first user message, the agent's own model) lands for the five main agents; sub-agents and stored agents are unaffected. For plain agents (`qa-android-agent` today) Mastra's native `generateTitle` finish hook does this; the pinned core's durable finish path never runs that hook, so the execution driver generates durable agents' titles itself (`generateFirstTurnTitle` in `agent/src/mastra/runs/execute.ts`, mirroring the native flow and persisting via `memory.createThread`). The title call bypasses the agent's context-bound inputProcessors and never fails the completed run (an oversized or hung title request is swallowed; the fallback stays). The client shows its `New conversation` fallback until then, and manual renames that land before first-turn completion — including while the title LLM call is in flight — suppress generation.
...
- **Limits.** The `prompt` is capped at 65,536 UTF-8 bytes (400 on overflow). Event buffers are bounded to 4 MiB / 10,000 events per run with oldest-eviction and an `evicted` flag — extremely long runs may replay with a gap in the middle, and the run summary carries `evicted: true` when that happened. Terminal runs stay replayable for 30 minutes, then only Mastra Memory messages remain. A watchdog force-fails running runs older than 30 minutes (fixed message, aborts the execution signal, releases the thread's active-run lock, and latches the cancel intent so the driver persists the partial turn at stream unwind — the terminal status stays `failed` with the watchdog message); any run API touch performs the reap — there is no background timer.
...
- `/knowledge` lists the signed-in user's Knowledge Base documents newest first with indexing status and management actions.
- `GET /api/storage/knowledge/documents` returns the user's document metadata after identity validation.
- `POST /api/storage/knowledge/documents` persists one raw attachment (multipart) plus a `processing` registry record, then fires the agent-side `knowledge-document-ingestion` workflow fire-and-forget; identity comes from the session, never the body.
- `GET /api/storage/knowledge/documents/[documentId]` returns one document's metadata after identity and ID validation.
- `DELETE /api/storage/knowledge/documents/[documentId]` fires the agent-side `knowledge-document-deletion` workflow (vectors → objects → metadata last); the record leaves the list only after retrieval can no longer return it.
- `GET /api/storage/knowledge/documents/[documentId]/original` returns the original upload's bytes after identity validation (storage key taken from verified metadata, never the URL).
- `POST /api/storage/knowledge/documents/[documentId]/retry` re-fires ingestion for `failed` documents and for `processing` records older than the stale-run window.
