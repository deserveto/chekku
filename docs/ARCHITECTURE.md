...
- **Durable pilot (pm-agent).** The PM Agent is registered as `createDurableAgent({ agent: pmAgent })` (public id and composition key unchanged), so its longest jobs (competitive research, up to 25 steps) run inside Mastra's built-in workflow engine with the default in-process PubSub and an in-memory event cache — no Redis. The run surface is unchanged: stop still flows through the run signal (forwarded to the durable run's internal controller), replay still comes from the run registry, and the execution driver calls the durable stream's `cleanup()` at terminal state so PubSub subscriptions and engine registry entries never leak. Workflow snapshots persist to Postgres through the shared `PostgresStore`, but crash recovery is not enabled (unavailable in the pinned `@mastra/core` 1.50.1, and re-driving abandoned runs would re-issue real LLM calls): a restart still abandons in-flight runs, and resume is conversational — prompting again in the same thread continues from the last persisted Memory context.
