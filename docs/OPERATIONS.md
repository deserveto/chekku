...
- The run registry is in-memory and single-instance. Restarting the agent server (dev or the `agent` container) kills in-flight runs; clients then see no active run and render the persisted Mastra Memory messages. When a run is **stopped**, the execution driver reconstructs the partial turn (user message, tool calls with bounded results, streamed text with a stop marker) from registry events and persists it to Memory best-effort, so the thread stays readable and prompting again in the same thread resumes from that context. Image attachments of a stopped multimodal turn are not retained (only a placeholder note), and a **failed** run still persists nothing.
- `pm-agent` executes through Mastra's durable workflow engine (pilot). Its snapshots persist in Postgres via the shared `PostgresStore`, but no automatic recovery runs at boot (unavailable in the pinned core version), so restarting the agent server still abandons an in-flight PM run. After a stop, prompting again in the same thread continues from the last persisted Memory context. Known engine limitation: an in-flight tool call is not interrupted by a stop — it runs to completion (tool calls are individually bounded, e.g. Reader reads cap at 30 seconds) and the abort lands at the next step; the chat UI flips pending tool cards to `interrupted` immediately on stop.
