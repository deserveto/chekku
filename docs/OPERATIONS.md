...
- `qa-browser-trajectory` — deterministic gate requiring the ordered `browser_goto`, pre-click `browser_snapshot`, a successful `browser_click`, and post-click `browser_snapshot` path, a same-origin `/pricing` destination reached after a click, and no prohibited interaction/navigation tools (`browser_type`, `browser_press`, `browser_select`, `browser_dialog`, `browser_drag`, `browser_evaluate`, `browser_back`, or `browser_tabs`). Benign extra clicks are allowed before the pricing navigation.
...
- `qa-report-vs-golden` — LLM judge scoring task coverage, evidence accuracy, scope safety, and report clarity against the committed golden reference. It uses `LLM_DEFAULT_MODEL` through the existing gateway by default; its 0.6 threshold is reported as `passed` or `scored`, and a deterministic gate failure is the only QA Web quality failure that makes the command exit non-zero.
...
Security note: the local fixture avoids a remote-site dependency, but this eval launches a real headless Chromium with unrestricted outbound network egress. Prompt instructions and trajectory checks are post-hoc evaluation controls, not a network sandbox. Run the command only in a trusted environment and do not use a browser profile containing sensitive credentials.
...
WEB_READER_BASE_URL=http://127.0.0.1:8081
# Per-user daily token quota for chat runs (input + output per user per UTC
# day; 0 = unlimited). Telegram channel turns and workflows are not metered.
TOKEN_DAILY_LIMIT=500000
```

The `weekly-social-drafts` workflow fires every Monday at 09:00 Asia/Jakarta via Mastra's built-in scheduler (no separate registration). One run drafts 2 base canonical-only Instagram posts plus, when the week contains a holiday, 1 bonus awareness post (total 2–3 `DRAFT` posts; each caption is derived later, at canonical approval). Base slots come from SearXNG trending research; when `WEB_READER_BASE_URL` is set, each chosen topic is also enriched with the self-hosted Reader's page markdown (single-page read per topic, parallel, bounded — per-topic fetch failure falls back to snippet only). Remaining base slots are filled from the deterministic evergreen-pillar rotation when research yields fewer than 2 topics. Trending results that overlap the chosen awareness day are skipped so the bonus and a base slot do not duplicate the same theme. Each post is drafted as a canonical-only DRAFT: the workflow routes through the `social-media-supervisor-agent` (the `[weekly-social-drafts]` marker makes it delegate straight to the Content Writer, which runs in canonical mode) to emit a platform-agnostic Canonical Content Unit. The canonical/repurpose mode is carried in `requestContext` (`SOCIAL_DRAFT_MODE_KEY`), not via an `instructions` override, so the supervisor's own routing instructions are never bypassed. The canonical unit alone is wrapped into `post.md` under the `canonical-unit` HTML comment delimiter and saved with status `DRAFT` to the fixed `social-media-agent` Garage namespace (the `SOCIAL_MEDIA_AGENT_ID` storage constant, decoupled from the agent identity) under `social-posts/<postId>/`. The caption is NOT drafted here — approving the canonical content in `/social-posts` fires the `repurpose-social-post` workflow, which calls the `social-media-content-writer` directly (repurpose mode, `instagram-writer` voice) to derive the Instagram caption, stores it in `caption.md`, and flips the post to `CANONICAL_APPROVED`. An unparseable canonical unit (empty or unstructured) is skipped before persistence and recorded in `researchNote`. The run then emails a review link per draft to `SOCIAL_DRAFT_REVIEW_EMAIL`.
...
The agent probe suite (`e2e/agents/agent-probe.spec.ts`) additionally exercises live LLM round trips, so it requires the agent server running with a reachable model gateway (`LLM_BASE_URL`, `LLM_API_KEY`, `LLM_DEFAULT_MODEL` in `agent/.env`) — start `npm run dev:agent` or the full dev stack first. It signs in a single test user, discovers the agents from the `/agents` catalog at runtime rather than from a hardcoded list (the catalog is the same merged, `HIDDEN_AGENT_IDS`-filtered view the user sees, so a future built-in is swept without editing the spec), and probes each `source: 'code'` (built-in) agent through the chat UI with the prompt `Apa yang kamu bisa lakukan untuk saya?`, asserting a non-empty, error-free reply within a per-agent budget derived from the probe's phase caps. `source: 'stored'` (user-created) cards are logged but deliberately skipped: opening a stored agent's chat can call `ensureStoredAgentUsesServerGateway` (`client/src/lib/stored-agents.ts`), which writes a real model-gateway migration to the shared stored-agent record, and its configured tools (e.g. Garage MCP) could run real side effects in response to the probe prompt — an automated regression sweep must not mutate someone's custom agent, and one half-configured stored agent must not fail coverage of the built-ins this suite exists to protect. Probe a specific stored agent manually (open its chat through the UI) when that coverage is needed. Because the catalog is only known once the browser is signed in, the sweep is one Playwright test with a `test.step` per built-in agent; a step that fails is reported red but does not abort the remaining agents, and the test's final assertion names every built-in agent that failed. `test.setTimeout` is set to a provisional upper bound before catalog discovery and refined afterwards once the built-in count is known — discovery itself races the ambient default test timeout (also `CATALOG_TIMEOUT_MS`, 30s) against the catalog-render expect, and without the provisional call the ambient timeout always wins first (its clock starts at the test's first line, before the expect's own clock starts), so the "is the agent server running?" diagnostic could never actually surface. The supervisor's delegation sub-agents (`social-media-content-writer`, `social-media-strategist-agent`, `visual-content-agent`) are in `HIDDEN_AGENT_IDS`, so the catalog renders no card for them and the sweep never reaches them — they are exercised only indirectly, when the supervisor delegates. That is a scope choice, not a technical limit: `normalizeAgentId` in `client/src/lib/chat-route.ts` validates only the shape of an id and never catalog membership, so `/chat?agent=visual-content-agent` opens a working chat against any registered agent, and all three answer the probe prompt normally. Widen the discovery step if the sweep should cover them. Probe conversations stay in the agent database under the test user's resource and are deliberately not cleaned up; only the `chekku_auth` user row is deleted afterwards. The suite adds one signup and one sign-in POST, which stays within the 5-per-60-seconds rate-limit caps even when run right after the auth suite.
