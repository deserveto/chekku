...
4. Password reset: request a link at `/forgot-password`; the reset link is valid for one hour and can be used once. In dev the link is printed to the server console when `RESEND_API_KEY` is unset.
