...
- New helpers in `storage/src/competitive-analyses.ts`:
  - `createShareToken(store, analysisId, anchorProduct): Promise<{ token: string; createdAt: string; anchorProduct: string }>` — idempotent and requires a nonblank `slides.md`. Reads existing token first; returns it as-is if already set. Creates new token only when none exists. Concurrent callers are serialized per analysis within one server process; adapters that report an `already-exists` race re-read and return the persisted winner. Cross-process writers remain subject to Garage v2.3's documented conditional-write limitation. Idempotency prevents token rotation from repeated button clicks.
  - `getShareToken(store, analysisId): Promise<{ token: string; createdAt: string } | undefined>` — returns `undefined` when no token file exists. Throws `ObjectStorageError` for any non-`not-found` storage error. The authenticated status seam maps outages to 503; the unauthenticated public route deliberately collapses every storage failure to 404 to avoid an existence oracle.
